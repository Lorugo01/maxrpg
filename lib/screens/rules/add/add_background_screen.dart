...
class AddBackgroundScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AddBackgroundScreen> createState() =>
      _AddBackgroundScreenState();
}

class _AddBackgroundScreenState extends ConsumerState<AddBackgroundScreen> {
  ...
  // Seleção estruturada de itens + PO (PHB 2024)
  ...
  // Seleção estruturada para PHB 2014
  ...
  final List<String> _selectedAbilityScores = [];
  ...
  Widget build(BuildContext context) {
    ...
    if (!isAdmin) {
      return Scaffold(
        ...
        body: const Center(
          child: Text('Apenas administradores podem adicionar antecedentes.'),
        ),
      );
    }
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Adicionar Antecedente'),
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                label: Text(_isLoading ? 'Salvando...' : 'Salvar Antecedente'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.orange,
                  foregroundColor: Colors.white,
  ...
  Future<void> _saveBackground() async {
    ...
    try {
      ...
      await SupabaseService.client.from('backgrounds').insert(backgroundData);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Antecedente adicionado com sucesso!'),
